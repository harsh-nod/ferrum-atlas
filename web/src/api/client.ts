...
export class RequestError extends Error {
  constructor(readonly code: string, message: string, readonly correlationId = '') {
    super(message);
...
export async function request<T>(path: string, signal: AbortSignal, body?: unknown): Promise<T> {
  ...
  if (!response.ok) {
    ...
    try { error = await response.json(); } catch { /* A proxy may return non-JSON errors. */ }
    throw new RequestError(error.code ?? `http_${response.status}`, error.message ?? `Request failed (${response.status})`, error.correlation_id);
  }
