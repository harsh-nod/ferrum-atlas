...
export function CoverageNotice({
  ...
  if (compact)
    return (
      <span className={`badge ${coverage.status}`}>
        <CircleHelp size={12} />
        {coverage.status}
      </span>
    );
  return (
    <section className="coverage-detail" aria-label="Analysis coverage">
      <div className="section-label">
        Analysis coverage{" "}
        <span className={`badge ${coverage.status}`}>{coverage.status}</span>
      </div>
      {coverage.reasons.length > 0 && (
        <ul className="reason-list">
          {coverage.reasons.map((item) => (
            <li key={item.reason}>
      ...
      {coverage.limitations.map((item, index) => (
    ...
    <div
      ...
      onPointerDown={(event) => {
        ...
        const move = (next: PointerEvent) =>
          onChange(
            clamp(value + (horizontal ? next.clientY : next.clientX) - origin),
          );
        const finish = () => {
