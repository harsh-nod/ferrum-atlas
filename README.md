...
The workspace separates model, capture, rust-analyzer adapter, storage, queries, imported evidence, HTTP and CLI ownership. TypeScript contracts are generated from Rust. See [local operations](docs/operations/local.md), [API contracts](docs/schema/api.md), [architecture decisions](docs/adr/0001-local-architecture.md), and [the tracked roadmap](https://github.com/harsh-nod/ferrum-atlas/issues).
